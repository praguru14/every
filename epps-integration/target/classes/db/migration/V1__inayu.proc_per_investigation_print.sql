...
--check flyway script execution
			...
			and ap.app_sr_no = v_last_inv_sr_no;
				...
				from(
					select pm.pno "personalNumber",
						...
						pd.rank_code "rankCode",
						(select r.name from inmdb.rank_mst r where r.code = pd.rank_code) "rankName",
						...
						'||QUOTE_NULLABLE(v_final_obs_remarks)||' "pastMedicalHistory",
						coalesce('||QUOTE_NULLABLE(v_curr_medi_catg)||',pd.medi_code) "presentMedicalCategory",
	...
	v_block_query := v_block_query|| '(select row_to_json(personnel) "appointmentPersonDetail"
			from(
				select pd.pno "personnelNumber",
					...
					pd.rank_cd "rankCode",
					(select r.name from inmdb.rank_mst r where r.code = pd.rank_cd) "rankName",
					...
					pd.past_medical_hist "pastMedicalHistory",
					pd.curr_medical_catg "presentMedicalCategory",
	...
	v_block_query:= v_block_query|| '(select row_to_json(row) "finalObservationDetail"
				from(select final_obs_remarks "finalObservationRemarks",
						...
						rank_cd "rankCode",
						curr_medi_catg "currentMedicalCategory" ,
