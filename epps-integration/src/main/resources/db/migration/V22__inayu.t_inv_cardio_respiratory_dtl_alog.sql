...
if not exists (select 1 from information_schema.triggers 
			   where trigger_schema ='inayu' and trigger_name = 'trg_inv_cardio_respiratory_dtl_alog') then
CREATE TRIGGER  trg_inv_cardio_respiratory_dtl_alog
    AFTER INSERT OR UPDATE 
    ON inayu.inv_cardio_respiratory_dtl
    FOR EACH ROW
    EXECUTE PROCEDURE inayu.t_inv_cardio_respiratory_dtl_alog();
end if;
