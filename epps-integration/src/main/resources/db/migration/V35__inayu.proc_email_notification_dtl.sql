...
begin
	...
	from (
			select pm.pno "personalNumber",
				...
				PGP_SYM_DECRYPT(pd.nud_email_id::bytea, current_setting('encrypt.key')) "to",
				(select PGP_SYM_DECRYPT(a.nud_email_id::bytea, current_setting('encrypt.key')) from inmdb.per_dtls a where a.pno = v_ama_pno) "from",
				null::text "cc",
				null::text "bcc",
	...
	if vin_ip_param ->>'vin_examination' ='1' then
		...
		from (
				...
				PGP_SYM_DECRYPT(pd.nud_email_id::bytea, current_setting('encrypt.key')) "to",
				PGP_SYM_DECRYPT(apd.nud_email_id::bytea, current_setting('encrypt.key'))  "from",
				null::text "cc",
				null::text "bcc",
				...
				/*concat('Dear ',pm.full_name,' (',pm.pno,') \n ',
					   ...
					   'Please contact your Authorized Medical Attendant (AMA) for other details.',' \n ', 
					    ...
					    (select u.name from inmdb.unit_mst u where u.code =apd.unit_code))*/null "mailContent",				
	...
	if vin_ip_param ->>'vin_request' ='1' then
		...
		from (
				...
				(select r.name from inmdb.rank_mst r where r.code = PGP_SYM_DECRYPT(pd.rank_code::bytea, current_setting('encrypt.key'))) "personnelRank",
				(select u.name from inmdb.unit_mst u where u.code =pd.unit_code) "personnelUnit",
				(select PGP_SYM_DECRYPT(an.full_name::bytea, current_setting('encrypt.key')) from inmdb.per_mst an where an.pno = ua.ama_pno) "amaName",
				 (select u.name from inmdb.unit_mst u where u.code =apd.unit_code) "amaUnit",
				...
				from 
				(select pd1.pno,
					to_date(to_char(now(),'yyyy')||lpad(rm.from_month::text,2,'0')||'01','yyyymmdd') from_dt,
					to_date(to_char(now(),'yyyy')||lpad(rm.to_month::text,2,'0')||'01','yyyymmdd')+interval '1 month' - interval '1 day' to_dt
				 from inmdb.per_dtls pd1
				 inner join inayu.rank_schedule_mst rm on PGP_SYM_DECRYPT(pd1.rank_code::bytea, current_setting('encrypt.key')) = rm.rank_code
				 where (to_date(to_char(now(),'yyyy')||lpad(rm.from_month::text,2,'0')||'01','yyyymmdd')- now()::date ) <=  31 
				 and not exists (select 1 from inayu.appointment_dtl ad where ad.pno = pd1.pno and ad.app_year = to_char(now(),'yyyy')::smallint)
				 
				 union all
				  
				 select pd2.pno,
					to_date(to_char(now()+interval '1 year','yyyy')||lpad(rm.from_month::text,2,'0')||'01','yyyymmdd') from_dt,
					to_date(to_char(now()+interval '1 year','yyyy')||lpad(rm.to_month::text,2,'0')||'01','yyyymmdd')+interval '1 month' - interval '1 day' to_dt
				 from inmdb.per_dtls pd2
				 inner join inayu.rank_schedule_mst rm on PGP_SYM_DECRYPT(pd2.rank_code::bytea, current_setting('encrypt.key')) = rm.rank_code
				 where (to_date(to_char(now()+interval '1 year','yyyy')||lpad(rm.from_month::text,2,'0')||'01','yyyymmdd')- now()::date ) <= 31 
				and not exists (select 1 from inayu.appointment_dtl ad where ad.pno = pd2.pno and ad.app_year = to_char(now()+interval '1 year','yyyy')::smallint)
				 ...
				 select pd3.pno,
					to_date(to_char(now()-interval '1 year','yyyy')||lpad(rm.from_month::text,2,'0')||'01','yyyymmdd') from_dt,
					to_date(to_char(now()-interval '1 year','yyyy')||lpad(rm.to_month::text,2,'0')||'01','yyyymmdd')+interval '1 month' - interval '1 day' to_dt
				 from inmdb.per_dtls pd3
				 inner join inayu.rank_schedule_mst rm on PGP_SYM_DECRYPT(pd3.rank_code::bytea, current_setting('encrypt.key')) = rm.rank_code
				 where (to_date(to_char(now()-interval '1 year','yyyy')||lpad(rm.from_month::text,2,'0')||'01','yyyymmdd')- now()::date ) <= 31 
				and not exists (select 1 from inayu.appointment_dtl ad where ad.pno = pd3.pno and ad.app_year = to_char(now()-interval '1 year','yyyy')::smallint)
				 
				 
				) q
				
				inner join inmdb.per_mst pm on q.pno = pm.pno
