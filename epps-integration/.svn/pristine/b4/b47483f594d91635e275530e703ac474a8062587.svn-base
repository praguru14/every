...
vin_investigation_block :=  replace(replace((replace (cast(vin_ip_param::json->>'vin_investigation_block' as character varying),'[','')),']',''),'"','''');
	...
	v_query := 'select p.unit_name as "Unit Name",a.pno as "Personnel No",p.rank_name as "Rank Name",
				...
				a.crea_by_nm as "Created By",a.created_dt "Created Date",
				...
				PGP_SYM_DECRYPT(gp.full_name::bytea, current_setting(''encrypt.key'')) as "Medical Specialist Name",
				g.log_dt "Investigation Block Date"
		...
		v_query := v_query||' left join inayu.per_mst_vw gp on g.log_by = gp.pno 
		where a.app_stage::text in('||replace(replace((replace (cast(vin_ip_param::json->>'vin_app_stage' as character varying),'[','')),']',''),'"','''')||') ';
		...
		v_query := (v_query||' and PGP_SYM_DECRYPT(p.rank_code::bytea, current_setting(''encrypt.key'')) in (' || vin_rank_code || ')');
		end if;
