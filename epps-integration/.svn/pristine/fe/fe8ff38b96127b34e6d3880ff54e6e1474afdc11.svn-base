...
raise notice '111 %',vin_ip_param ->> 'vin_app_no';
	if vin_ip_param ->> 'vin_app_no' is not null then 
		...
		select (to_date(to_char(now(),'yyyy')||lpad(r.to_month::text,2,'0')||'01','yyyymmdd')+interval'4 months'-interval'1 day')::date ,
		(to_date(to_char(now(),'yyyy')||lpad(r.to_month::text,2,'0')||'01','yyyymmdd')+interval'1 months'-interval'1 day')::date 
		...
		v_due_date := v_apply_before_date;
	end if;
...
raise notice 'v_age_yy %',v_age_yy;
	...
	from ( select ';
if coalesce((vin_ip_param ->> 'vin_edossier'),'0')<> '1' then
